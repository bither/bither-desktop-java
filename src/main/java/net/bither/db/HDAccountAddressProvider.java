...
public class HDAccountAddressProvider implements IHDAccountAddressProvider {
    ...
    private static HDAccountAddressProvider txProvider = new HDAccountAddressProvider(ApplicationInstanceManager.txDBHelper);
    ...
    public HDAccountAddressProvider(TxDBHelper db) {
        this.mDb = db;
    }
    ...
    public int issuedIndex(int hdAccountId, AbstractHD.PathType pathType) {
        ...
        try {
            PreparedStatement statement = this.mDb.getPreparedStatement("select ifnull(max(address_index),-1) address_index from "
                    ...
                    new String[]{Integer.toString(pathType.getValue()), "1", Integer.toString(hdAccountId)});
    ...
    public int allGeneratedAddressCount(int hdAccountId, AbstractHD.PathType pathType) {
        ...
        try {
            PreparedStatement statement = this.mDb.getPreparedStatement("select ifnull(count(address),0) count from "
                            + AbstractDb.Tables.HD_ACCOUNT_ADDRESS + " where path_type=? and hd_account_id=? ",
    ...
    public void updateIssuedIndex(int hdAccountId, AbstractHD.PathType pathType, int index) {
        ...
        try {
            String[] params = new String[]{
                    "1", Integer.toString(pathType.getValue()), Integer.toString(index), Integer.toString(hdAccountId)
            };
    ...
    public String externalAddress(int hdAccountId) {
        ...
        try {
            PreparedStatement statement = this.mDb.getPreparedStatement("select address from " + AbstractDb.Tables.HD_ACCOUNT_ADDRESS
                    ...
                    new String[]{Integer.toString(AbstractHD.PathType.EXTERNAL_ROOT_PATH.getValue()), "0", Integer.toString(hdAccountId)});
            ResultSet cursor = statement.executeQuery();
            if (cursor.next()) {
    ...
    public HashSet<String> getBelongAccountAddresses(int hdAccountId, List<String> addressList) {
        ...
        try {
            String sql = Utils.format("select address from hd_account_addresses where hd_account_id=? address in (%s) "
                    , Utils.joinString(temp, ","));
            PreparedStatement statement = this.mDb.getPreparedStatement(sql,
                    new String[] {Integer.toString(hdAccountId)});
            ResultSet cursor = statement.executeQuery();
            while (cursor.next()) {
                int idColumn = cursor.findColumn(AbstractDb.HDAccountAddressesColumns.ADDRESS);
                if (idColumn != -1) {
        ...
        if (addressList != null && addressList.size() > 0) {
            ...
            try {
                ...
                while (cursor.next()) {
                    ...
                    int hdAccountId = cursor.getInt(2);
    ...
    public List<Integer> getRelatedHDAccountIdList(List<String> addresses) {
        ...
        if (addresses != null && addresses.size() > 0) {
            ...
            try {
                ...
                while (c.next()) {
                    hdAccountIdList.add(c.getInt(1));
                }
                c.close();
                statement.close();
            } catch (SQLException e) {
    ...
    public HDAccount.HDAccountAddress addressForPath(int hdAccountId, AbstractHD.PathType type, int index) {
        ...
        try {
            PreparedStatement statement = this.mDb.getPreparedStatement("select address,pub,path_type,address_index,is_issued,is_synced from " +
                            AbstractDb.Tables.HD_ACCOUNT_ADDRESS + " where path_type=? and address_index=? and hd_account_id=? ",
    ...
    public List<byte[]> getPubs(int hdAccountId, AbstractHD.PathType pathType) {
        ...
        try {
            PreparedStatement statement = this.mDb.getPreparedStatement("select pub from hd_account_addresses where path_type=? and hd_account_id=? ",
                    new String[]{Integer.toString(pathType.getValue()), Integer.toString(hdAccountId)});
            ResultSet cursor = statement.executeQuery();
            while (cursor.next()) {
                try {
    ...
    public int unSyncedAddressCount(int hdAccountId) {
        ...
        try {
            ...
            PreparedStatement statement = this.mDb.getPreparedStatement(sql, new String[]{"0", Integer.toString(hdAccountId)});
            ResultSet cursor = statement.executeQuery();
            if (cursor.next()) {
    ...
    public void updateSyncedForIndex(int hdAccountId, AbstractHD.PathType pathType, int index) {
        this.mDb.executeUpdate("update hd_account_addresses set is_synced=? where path_type=? and address_index>? and hd_account_id=? "
                , new String[]{"1", Integer.toString(pathType.getValue()), Integer.toString(index), Integer.toString(hdAccountId)});
            ...
            for (In in : inList) {
                ...
                PreparedStatement statement = this.mDb.getPreparedStatement(sql,
                        new String[]{Base58.encode(in.getPrevTxHash()),
                                Integer.toString(outPoint.getOutSn()), Integer.toString(hdAccountId)});
...
//                    " order by b.tx_hash ,b.in_sn";
            statement = this.mDb.getPreparedStatement(sql, new String[] {Integer.toString(hdAccountId)});
            c = statement.executeQuery();
            while (c.next()) {
...
//                " limit ? ";
        try {
            PreparedStatement statement = this.mDb.getPreparedStatement(sql,
                    new String[]{Integer.toString(greateThanBlockNo)
